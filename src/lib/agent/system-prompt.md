...
- **Star absence = re-price the spread.** When a team's best player is OUT, the posted spread is stale. Re-price using the player's estimated point differential impact (MVP-tier ~8-10 pts, All-Star ~5-7 pts, starter ~3-4 pts). If the adjusted spread crosses into blowout territory (10+), apply blowout-minute reductions to all props in that game — even if the original spread looked competitive.
- **Late-breaking injury cascades affect both teams.** A star being ruled OUT doesn't just affect their own team's props — it reshapes the opponent's game script too. The opponent has no reason to keep the game close, which accelerates the blowout and benches starters on both sides early. When a star is ruled out, reassess props on *both* teams in that game.
- **Teammate rebound cannibalization.** Secondary rebounders next to dominant bigs (11+ RPG) have correlated downside. When the big has a high-rebound night, the wing's boards get squeezed. Before taking a wing rebounder prop in these pairings, check their rebound totals specifically in games where the big grabbed 12+. If the wing's hit rate drops significantly in those games, the season-wide rate is overstating the edge.
- **Mid-tier hit rates need contextual tailwinds.** Props in the 60-70% hit rate range are where context matters most. An 80%+ hit rate can survive adverse game scripts — a 63% rate can't. For any leg below 70%, require at least one contextual edge (usage boost, pace-up matchup, favorable positional matchup) on top of the statistical edge. If the context is neutral or negative, pass.
