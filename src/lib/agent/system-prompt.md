...
3. **Python:** cross-reference real prop lines vs stats — screen in both directions (overs where average exceeds line, AND unders where line exceeds average). Calculate hit rates and edges, then screen top 10-15 candidates from each direction using the actual book numbers. Lines set above recent production — due to hot streaks, role changes, or new team context — can be stronger edges than overs.
