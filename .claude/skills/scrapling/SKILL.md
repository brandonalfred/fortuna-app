...
PrizePicks displays player prop projections as a rendered SPA with anti-bot protection (PerimeterX + DataDome). Use `StealthyFetcher` to bypass these protections.

**URL pattern**: `https://app.prizepicks.com/board/{sport}` — swap `{sport}` for: `nba`, `nfl`, `mlb`, `nhl`, `soccer`, etc.

**Fetcher**: `StealthyFetcher` with `headless=True`, `network_idle=True`, `wait_selector="ul[aria-label='Projections List']"`, `timeout=30000`

**Selectors**:
- Projections list container: `ul[aria-label='Projections List']`
- Individual projection cards: `ul[aria-label='Projections List'] > li`
- Player name within card: `h3`
- All text in card: `*::text` (using `.getall()`)
- Each `<li>` also has `aria-label="Player Name - TEAM"` and `id="test-projection-li"`

**Card text pattern** — each card's `*::text` produces an array:

| Index | Field | Example |
|-------|-------|---------|
| 0 | popularity | `153.7K` |
| 1 | team-pos | `LAC - F`, `NYK - C-F` |
| 2 | player_name | matches `<h3>` text |
| 3 | home/away | `vs` (home) or `@` (away) |
| 4 | opponent | team abbreviation |
| 5 | game_time | `Mon 9:12pm` |
| 6 | line_value | `19.5` |
| 7 | stat_type | `Points`, `Rebs+Asts`, `PRA` |
| 8 | `Less` | optional (missing on most popular card) |
| 9 | `More` | |

**Stat categories**: Points, Rebounds, Assists, PRA, Rebs+Asts, 3-PT Made, Fantasy Score, FG Made, Offensive Rebounds, Defensive Rebounds, 2-PT Made

**Live games caveat**: In-progress games embed scores into the text array (e.g., `@ UTA 59`, `Q2 00:00`), shifting indices. Filter these out or handle with a separate parsing path.
